...
Sophus::SE3f System::TrackLidarStereo(const cv::Mat &imLeft, const cv::Mat &imRight, std::unordered_map<std::pair<int, int>, float> &lidarDepth, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=LIDAR_STEREO)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Lidar-Stereo." << endl;
        exit(-1);
    }
    ...
    // std::cout << "start GrabImageStereo" << std::endl; // TODO add lidar info here
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename);
